...
int distance( pix *p1, struct box *box1, pix *p2, struct box *box2, int cs) {
  ...
  for(y = 0; y < dy; y++) {
    for(x = 0; x < dx; x++) {	// try global shift too ???
      v1 = ((getpixel (p1, x1 + x, y1 + y) < cs)?1:0);
      i1=8;	// better gray?
      
      v2 = ((getpixel (p2, x2 + x, y2 + y) < cs)?1:0);
      i2=8;	// better gray?
...
/*
    ...
    TIME_STOP
*/
    struct return_element *returnArr; 
    returnArr = deviceFuncCall (job, pp);

    for (i = 0; i < ni; i++) {
      if (returnArr[i].j_max == -1) {
        continue;
      }

      box4 = returnArr[i].box4.box;
      box2 = returnArr[i].box2.box;
      bc = returnArr[i].bc;
      dist = returnArr[i].dist;
